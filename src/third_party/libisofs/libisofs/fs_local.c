...
char* lfs_get_path(IsoFileSource *src)
{
    ...
    if (data->parent == src) {
        ...
        if (name_len >= 2 && data->name[1] == ':' && data->name[name_len-1] != '/') {
            char* root = malloc(name_len + 2);
            if (root == NULL)
                return NULL;
            strcpy(root, data->name);
            /* Ensure Windows root ends with separator */
            strcat(root, "/");
            return root;
        }
        ...
        /* Unified logic for adding separator */
#ifdef _WIN32
        // Windows: don't add if already ends with \ or /
        if (pathlen > 0 && path[pathlen-1] != '/') {
#else        
        if (pathlen != 1) {
            /* Unix: pathlen can only be 1 for root */
#endif
            path[pathlen] = '/';
            path[pathlen + 1] = '\0';
        }

        path = strcat(path, data->name);
#ifdef _WIN32
        // Windows: Convert to Unix-style path
        convert_path_to_unix(path);
#endif
        return path;
    }
}
...
#ifdef _DEBUG
    printf("lfs_get_by_path: fs=%p, path=%s, file=%p\n", fs, path, file);
#endif
...
#ifdef _WIN32
    if (!strcmp(path, "/") || !strcmp(path, "") || (strlen(path) == 2 && path[1] == ':')) {
#else
    if (!strcmp(path, "/")) {
#endif
#ifdef _DEBUG
        printf("lfs_get_by_path: root /\n");
#endif
